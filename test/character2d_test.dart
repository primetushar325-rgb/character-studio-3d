...
void main() {
  ...
  group('The three original characters', () {
    test('exactly three built-ins with §32 metadata', () {
      expect(CharacterCatalog.builtIn.length, 3);
      final ids = CharacterCatalog.builtIn.map((c) => c.id).toList();
      expect(ids, ['bd_farmer_male', 'village_girl', 'school_teacher']);
      for (final spec in CharacterCatalog.builtIn) {
        final m = spec.metadata();
        expect(m['type'], '2d_skeletal');
        expect(m['rig'], 'humanoid_v1');
    ...
    test('same universe but clearly different (palette, face, silhouette)', () {
      ...
      expect(g.face.eyeRx, greaterThan(f.face.eyeRx)); // girl's bigger eyes
      expect(f.build(f.defaultPalette, {}), isNot(equals(g.build(g.defaultPalette, {}))));
      // Every character exposes a different set of separable parts.
      expect(f.build(f.defaultPalette, {}).length, greaterThan(15));
      expect(g.build(g.defaultPalette, {}).length, greaterThan(15));
      expect(t.build(t.defaultPalette, {}).length, greaterThan(15));
    });
    ...
    test('all three characters paint without errors in every key pose', () {
      ...
      expect(painted, 240); // 3 chars × 8 poses × 10 expressions
      recorder.endRecording().dispose();
    });

    test('part painters receive a valid ordered list (z-stable)', () {
      final parts = orderParts(CharacterCatalog.farmer.build(CharacterCatalog.farmer.defaultPalette, {}));
