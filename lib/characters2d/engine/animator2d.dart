...
class PuppetAnimator {
  PuppetAnimator({required this.rig, int seed = 11})
      : rigKind = rig.kind,
        sm = StateMachine2D();
  ...
  Map<String, Clip2D> get clips => ClipLibrary.forRig(rigKind);
  ...
  /// Absolute time inside the current base clip (seconds).
  double get clipTime => _t;
  double get clipDuration => (clips[_clipId] ?? ClipLibrary.get(_clipId)).duration;
  ...
  PuppetFrameData update(double dt) {
    ...
    if (_fade > 0 && _prevClipId != null) {
      final prev = clips[_prevClipId!] ?? ClipLibrary.get(_prevClipId!);
    ...
    if (_talking) {
      final talkClip = clips['talk'] ?? ClipLibrary.get('talk');
      final tl = talkClip.sample(_clock % talkClip.duration);
      tl.onto(talkBase);
    }
    ...
    var gesture = Pose2D();
    if (_gestureId != null) {
      final g = clips[_gestureId!] ?? ClipLibrary.get(_gestureId!);
      _gestureT += dt;
    ...
    // Quadruped speech drives the jaw bone.
    if (rig.byName.containsKey('jaw')) {
      combined.angles['jaw'] = (combined.angles['jaw'] ?? 0) + face.mouthOpen * 26;
    }
